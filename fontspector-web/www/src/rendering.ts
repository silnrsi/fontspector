...
CheckSpecificRendering["outline_alignment_miss"] = function (
  metadata,
  font: FontInfo,
) {
  let misses: AlignmentMisses = metadata;
  let per_glyph_misses = misses.reduce(
    (acc: Record<string, AlignmentMisses>, miss) => {
  ...
  for (var glyph of Object.keys(per_glyph_misses)) {
    ...
    for (var miss of misses) {
      ...
      if (!doneLines[miss.line]) {
        ...
        svg
          .line(bounds.x, miss.y_expected, bounds.x2, miss.y_expected)
          .stroke({ color: "blue", width: 1 });
        svg
          .text(miss.line)
          .move(bounds.cx, miss.y_expected - 20)
          .font({ size: 20 })
          .fill("blue")
          .flip("y");
      }
    ...
    svg.translate(0, -(svg.transform().translateY ?? 0));
    svg.size(200, 200);
    reports += `
            <div class="row">
                <div class="col-sm">
